...
> 1、本人实际生产项目中使用了大量定时任务需要进行管理和测试
>
> 2、项目中通过硬编码的方式实现了基于 spring 框架的定时任务实现立即执行及CRUD
>
> 3、鉴于平安银行内部发起的基于 javaagent 定时任务实现立即执行专利的申请
>
> 4、结合相关知识开发了一套通用的定时任务立即执行及CRUD的组件，并提供多种实现方式，供热爱技术的开发人员进行参考
